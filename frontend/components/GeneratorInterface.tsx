                ...
                <CardContent className="p-3 pt-0 space-y-3">
                  ...
                  {promptVariables.length > 0 && promptVariables.map((variable) => (
                    <div key={variable.id} className="space-y-1.5">
                      <Label className="text-xs text-foreground">{variable.label}</Label>
                      <div className="flex items-center gap-2">
                        <Input
                  ...
                  <CardContent className="p-3 pt-0 space-y-2">
                    <div className="space-y-1 text-xs">
                      <div className="flex justify-between">
                        ...
                        <span className="font-mono text-foreground">{evilSlider[0]}%</span>
                      </div>
                      <div className="flex justify-between">
                        <span className="text-muted-foreground">Middle Finger</span>
                        <span className="font-mono text-foreground">{middleFinger ? "Yes" : "No"}</span>
                      </div>
                      <div className="flex justify-between">
                        <span className="text-muted-foreground">Camera Effects</span>
                        <span className="font-mono text-foreground">{cameraEffects.length > 0 ? cameraEffects.join(", ") : "None"}</span>
                      </div>
                      <div className="flex justify-between">
                        <span className="text-muted-foreground">Model</span>
                        <span className="font-mono text-foreground">Nano Banana Pro</span>
                      </div>
                      <div className="flex justify-between">
                        <span className="text-muted-foreground">Aspect Ratio</span>
                        <span className="font-mono text-foreground">{aspectRatio}</span>
                      </div>
                      <div className="flex justify-between">
        ...
        <div className="w-20 lg:w-24 border-l border-border/50 bg-card/30 shrink-0 flex flex-col overflow-hidden">
          ...
          <div className="flex-1 overflow-y-auto p-1.5 space-y-1.5">
            {variations.map((variation) => (
              <div
                key={variation.id}
                className={`relative aspect-square rounded-sm overflow-hidden cursor-pointer transition-all ${selectedVariation === variation.id
